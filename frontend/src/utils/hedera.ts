...
export const initializeClient = (network: string): Client => {
  let client: Client;
...
export async function evmAddressToAccountId(
  ...
  try {
    ...
    if (response.data && response.data.account) {
      ...
      console.log(
        `Hedera Account ID for EVM address ${formattedAddress}: ${accountId}`
      );
      return accountId;
    }

    throw new Error(
      `No Hedera account found for EVM address: ${formattedAddress}`
    );
  } catch (error) {
    console.error(
      `Error fetching account ID for EVM address ${evmAddress}:`,
      error
    );
    throw error;
  }
...
export function accountIdToSolidityAddress(accountId: string): string {
  try {
    ...
    console.log(
      `Solidity Address for Account ID ${accountId}: ${solidityAddress}`
    );
    return solidityAddress;
  } catch (error) {
    console.error(
      `Error converting Account ID ${accountId} to Solidity address:`,
      error
    );
    throw error;
  }
...
export async function accountIdToEvmAddress(
  ...
  try {
    ...
    console.error(
      `Error fetching EVM address for Account ID ${accountId}:`,
      error
    );
    throw error;
  }
